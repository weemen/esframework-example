...
from api.application.commands import StartGame, GuessLetter, GuessWord
from api.domain.events import GameStarted, LetterGuessed, LetterNotGuessed, WordGuessed, GameWon, GameLost
...
class Game(AggregateRoot):
    ...
    def guess_letter(self, command: GuessLetter):
        ...
        if command.get_letter() in list(self.__word):
            ...
            if self.__tries == 0:
                self.apply(GameLost(command.get_aggregate_root_id()))
    ...
    def apply_letter_guessed(self, event: LetterGuessed):
        ...
        if len(set(self.__word)) == len(set(self.__letters_guessed)):
            self.apply(GameWon(event.get_aggregate_root_id()))
    ...
    def guess_word(self, command: GuessWord):
        ...
        if command.get_word() == self.__word:
            self.apply(
